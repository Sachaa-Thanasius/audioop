...
// static HPy
// audioop_max_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
//     }
//     return HPyLong_FromUnsignedLong(ctx, max);
// }
...
// static HPy
// audioop_minmax_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
//         if (val < min) min = val;
//     }
//     return HPy_BuildValue(ctx, "(ii)", min, max);
// }
...
// static HPy
// audioop_avg_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
//     return HPyLong_FromLong(ctx, avg);
// }
...
// static HPy
// audioop_rms_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
//     return HPyLong_FromUnsignedLong(ctx, res);
// }

// static double _sum2(const int16_t *a, const int16_t *b, HPy_ssize_t len)
// {
//     HPy_ssize_t i;
//     double sum = 0.0;
...
// static HPy
// audioop_findfit_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment,
//                      HPy_buffer *reference)
// /*[clinic end generated code: output=5752306d83cbbada input=62c305605e183c9a]*/
// {
//     const int16_t *cp1, *cp2;
//     HPy_ssize_t len1, len2;
//     HPy_ssize_t j, best_j;
//     double aj_m1, aj_lm1;
...
//     return HPy_BuildValue(ctx, "(nf)", best_j, factor);
// }
...
// static HPy
// audioop_findfactor_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment,
//                         HPy_buffer *reference)
...
// static HPy
// audioop_findmax_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment,
//                      HPy_ssize_t length)
...
//     return HPyLong_FromSsize_t(ctx, best_j);
// }
...
// static HPy
// audioop_avgpp_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
//     return HPyLong_FromUnsignedLong(ctx, avg);
// }
...
// static HPy
// audioop_maxpp_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
// static HPy
// audioop_cross_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
//         prevval = val;
//     }
//     return HPyLong_FromSsize_t(ctx, ncross);
// }
...
// static HPy
// audioop_mul_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width,
//                  double factor)
...
//     rv = HPyBytes_FromStringAndSize(ctx, NULL, fragment->len);

//     if (HPy_IsNull(rv))
//         return HPy_NULL;
//     ncp = (signed char *)HPyBytes_AsString(ctx, rv);
...
// static HPy
// audioop_tomono_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width,
...
//     cp = fragment->buf;
//     len = fragment->len;
//     if (!audioop_check_parameters(ctx, module, len, width))
//         return HPy_NULL;
//     if (((len / width) & 1) != 0) {
//         HPy h_AudioopError = HPyGlobal_Load(ctx, AudioopError);
//         HPyErr_SetString(ctx, h_AudioopError,
...
//     rv = HPyBytes_FromStringAndSize(ctx, NULL, len/2);
//     if (HPy_IsNull(rv))
//         return HPy_NULL;
//     ncp = (signed char *)HPyBytes_AsString(ctx, rv);
...
// static HPy
// audioop_tostereo_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width,
//                       double lfactor, double rfactor)
...
//     if (fragment->len > HPY_SSIZE_T_MAX/2) {
//         HPyErr_SetString(ctx, ctx->h_MemoryError,
//                         "not enough memory for output buffer");
//         return HPy_NULL;
//     }

//     rv = HPyBytes_FromStringAndSize(ctx, NULL, fragment->len*2);
//     if (HPy_IsNull(rv))
//         return HPy_NULL;
//     ncp = (signed char *)HPyBytes_AsString(ctx, rv);
...
// static HPy
// audioop_add_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment1,
//                  HPy_buffer *fragment2, int width)
...
//     if (!audioop_check_parameters(ctx, module, fragment1->len, width))
//         return HPy_NULL;
...
//     rv = HPyBytes_FromStringAndSize(ctx, NULL, fragment1->len);
//     if (HPy_IsNull(rv))
//         return HPy_NULL;
//     ncp = (signed char *)HPyBytes_AsString(ctx, rv);
...
// static HPy
// audioop_bias_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width, int bias)
...
// static HPy
// audioop_reverse_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
//     if (HPy_IsNull(rv))
//         return HPy_NULL;
//     ncp = (unsigned char *)HPyBytes_AsString(ctx, rv);
...
// static HPy
// audioop_byteswap_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
// static HPy
// audioop_lin2lin_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width,
//                      int newwidth)
...
//     if (!audioop_check_parameters(ctx, module, fragment->len, width))
//         return HPy_NULL;
//     if (!audioop_check_size(ctx, module, newwidth))
//         return HPy_NULL;

//     if (fragment->len/width > HPY_SSIZE_T_MAX/newwidth) {
//         HPyErr_SetString(ctx, ctx->h_MemoryError,
//                         "not enough memory for output buffer");
//         return HPy_NULL;
//     }
//     rv = HPyBytes_FromStringAndSize(ctx, NULL, (fragment->len/width)*newwidth);
//     if (HPy_IsNull(rv))
//         return HPy_NULL;
//     ncp = (unsigned char *)HPyBytes_AsString(ctx, rv);
...
// static HPy
// audioop_ratecv_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width,
//                     int nchannels, int inrate, int outrate, HPy state,
...
//     HPy samps, str, rv = HPy_NULL, channel;
//     int bytes_per_frame;

//     if (!audioop_check_size(ctx, module, width))
//         return HPy_NULL;
//     if (nchannels < 1) {
//         HPy h_AudioopError = HPyGlobal_Load(ctx, AudioopError);
//         HPyErr_SetString(ctx, h_AudioopError,
...
//     if (HPy_Is(ctx, state, ctx->h_None)) {
//         d = -outrate;
//         for (chan = 0; chan < nchannels; chan++)
//             prev_i[chan] = cur_i[chan] = 0;
//     }
//     else {
//         if (!HPyTuple_Check(ctx, state)) {
//             HPyErr_SetString(ctx, ctx->h_TypeError, "state must be a tuple or None");
//             goto exit;
//         }
...
//             goto exit;
//         if (HPy_Length(ctx, samps) != nchannels) {
//             HPy h_AudioopError = HPyGlobal_Load(ctx, AudioopError);
//             HPyErr_SetString(ctx, h_AudioopError,
...
//         }
//         for (chan = 0; chan < nchannels; chan++) {
//             channel = HPy_GetItem_i(ctx, samps, chan);
//             if (!HPyTuple_Check(ctx, channel)) {
...
//     if (len == 0)
//         str = HPyBytes_FromStringAndSize(ctx, NULL, 0);
//     else {
...
//         if (outrate > HPY_SSIZE_T_MAX / q / bytes_per_frame)
//             str = HPy_NULL;
//         else
//             str = HPyBytes_FromStringAndSize(ctx, NULL,
...
//         goto exit;
//     }
//     ncp = HPyBytes_AsString(ctx, str);
//     cp = fragment->buf;

//     for (;;) {
//         while (d < 0) {
//             if (len == 0) {
//                 HPyTupleBuilder builder = HPyTupleBuilder_New(ctx, nchannels);
//                 for (chan = 0; chan < nchannels; chan++) {
//                     HPy h_item = HPy_BuildValue(ctx, "(ii)", prev_i[chan], cur_i[chan]);
//                     HPyTupleBuilder_Set(ctx, builder, chan, h_item);
//                     HPy_Close(ctx, h_item);
//                 }
//                 HPy samps = HPyTupleBuilder_Build(ctx, builder);
...
//                 len = (HPy_ssize_t)(ncp - HPyBytes_AsString(ctx, str));
//                 rv = HPyBytes_FromStringAndSize(ctx, HPyBytes_AsString(ctx, str), len);
//                 Py_SETREF(str, rv);
//                 if (HPy_IsNull(str))
//                     goto exit;
//                 rv = HPy_BuildValue(ctx, "(O(iO))", str, d, samps);
//                 HPy_Close(ctx, samps);
//                 HPy_Close(ctx, str);
//                 goto exit; /* return rv */
//             }
...
// static HPy
// audioop_lin2ulaw_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
//     rv = HPyBytes_FromStringAndSize(ctx, NULL, fragment->len/width);
//     if (HPy_IsNull(rv))
//         return HPy_NULL;
//     ncp = (unsigned char *)HPyBytes_AsString(ctx, rv);
...
// static HPy
// audioop_ulaw2lin_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
//     if (fragment->len > HPY_SSIZE_T_MAX/width) {
//         HPyErr_SetString(ctx, ctx->h_MemoryError,
//                         "not enough memory for output buffer");
//         return HPy_NULL;
//     }
...
// static HPy
// audioop_lin2alaw_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
// static HPy
// audioop_alaw2lin_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width)
...
// static HPy
// audioop_lin2adpcm_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width,
//                        HPy state)
// /*[clinic end generated code: output=cc19f159f16c6793 input=12919d549b90c90a]*/
// {
//     signed char *ncp;
//     HPy_ssize_t i;
//     int step, valpred, delta, index, sign, vpdiff, diff;
//     HPy rv = HPy_NULL, str;
...
//         index = 0;
//     }
//     else if (!HPyTuple_Check(ctx, state)) {
//         HPyErr_SetString(ctx, ctx->h_TypeError, "state must be a tuple or None");
//         return HPy_NULL;
//     }
//     else if (!HPyArg_Parse(ctx, NULL, state, "ii;lin2adpcm(): illegal state argument",
//                                &valpred, &index))
//     {
//         return HPy_NULL;
//     }
...
//         HPyErr_SetString(ctx, ctx->h_ValueError, "bad state");
//         return HPy_NULL;
//     }

//     str = HPyBytes_FromStringAndSize(ctx, NULL, fragment->len/(width*2));
//     if (HPy_IsNull(str))
//         return HPy_NULL;
//     ncp = (signed char *)HPyBytes_AsString(ctx, str);
...
//     }
//     rv = HPy_BuildValue(ctx, "(O(ii))", str, valpred, index);
//     HPy_Close(ctx, str);
//     return rv;
// }
...
// static HPy
// audioop_adpcm2lin_impl(HPyContext *ctx, HPy module, HPy_buffer *fragment, int width,
//                        HPy state)
// /*[clinic end generated code: output=3440ea105acb3456 input=f5221144f5ca9ef0]*/
// {
//     signed char *cp;
//     signed char *ncp;
//     HPy_ssize_t i, outlen;
...
//     if (fragment->len > (HPY_SSIZE_T_MAX/2)/width) {
//         HPyErr_SetString(ctx, ctx->h_MemoryError,
//                         "not enough memory for output buffer");
//         return HPy_NULL;
//     }
//     outlen = fragment->len*width*2;
//     str = HPyBytes_FromStringAndSize(ctx, NULL, outlen);
//     if (HPy_IsNull(str))
//         return HPy_NULL;
//     ncp = (signed char *)HPyBytes_AsString(ctx, str);
//     cp = fragment->buf;
