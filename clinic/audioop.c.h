...
audioop_getsample_test_impl(HPyContext *ctx, HPy module, const HPy *args, size_t nargs)
{
    ...
    PyObject *py_buff = HPy_AsPyObject(ctx, args[0]);
    if (PyObject_GetBuffer(py_buff, &fragment, PyBUF_SIMPLE) != 0) {
        goto exit;
    }
    if (!PyBuffer_IsContiguous(&fragment, 'C')) {
        _LTS_PyArg_BadArgument("getsample", "argument 1", "contiguous buffer", py_buff);
        goto exit;
    }

    width = HPyLong_AsInt64_t(ctx, args[1]);
    ...
    return_value = audioop_getsample_impl(ctx, module, (HPy_buffer *) &fragment, width, index);
